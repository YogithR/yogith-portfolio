...
export type ProjectEntry = {
  ...
  imageSrc: string;
  /** How the image fills its frame. Use `"contain"` for full posters that should not be cropped. */
...
export const projects: readonly ProjectEntry[] = [
  {
    ...
    summary:
      "Built a fully deployed autonomous ML platform where 5 specialized AI agents collaborate to train, optimize, and explain machine learning models from raw CSV data — no data science knowledge required. Directly competes with DataRobot ($2,500/month) as a free open-source alternative with an LLM reasoning layer no existing AutoML tool has.",
    tech: [
      "Python",
      "LangGraph",
      "Groq API",
    ...
    highlights: [
      "88.5% accuracy on Heart Disease prediction dataset",
      "85.3% accuracy on 145,460-row Rain in Australia dataset",
      "5 ML models trained simultaneously in parallel using ThreadPoolExecutor",
      "Bayesian hyperparameter tuning via Optuna — modeled as RL reward loop",
      "SHAP explainability — identifies top features driving every prediction",
      "ChromaDB RAG memory — agents learn from past experiments over time",
